...
def _check_conjunto_access(cur, user: dict, conjunto_id: int):
    """SA passes if building is in their org; admin passes if assigned to the building; others → 403."""
    rol = user.get("rol")
    if rol == "superadmin":
        org_id = user.get("organizacion_id")
        if org_id:
            cur.execute("SELECT organizacion_id FROM conjuntos WHERE id = %s", (conjunto_id,))
            row = cur.fetchone()
...
def get_conjunto(conjunto_id: int):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT e.*,
...
def create_conjunto(data: ConjuntoCreate):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO conjuntos (nombre, direccion, pisos) VALUES (%s,%s,%s) RETURNING *",
...
def update_conjunto(conjunto_id: int, data: ConjuntoCreate):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE conjuntos SET nombre=%s, direccion=%s, pisos=%s WHERE id=%s RETURNING *",
                (data.nombre, data.direccion, data.pisos, conjunto_id),
            )
            row = cur.fetchone()
            if not row:
                raise HTTPException(status_code=404, detail="Conjunto no encontrado")
...
def list_torres(conjunto_id: int):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT t.*,
...
@router.post("/{conjunto_id}/torres", status_code=201)
def create_torre(conjunto_id: int, data: TorreCreate, current_user: dict = Depends(get_current_user)):
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            if not cur.fetchone():
                raise HTTPException(status_code=404, detail="Conjunto no encontrado")
            cur.execute(
                "INSERT INTO torres (conjunto_id, nombre, numero, pisos, tipo) VALUES (%s,%s,%s,%s,%s) RETURNING *",
                (conjunto_id, data.nombre, data.numero, data.pisos, data.tipo),
            )
            return dict(cur.fetchone())


@router.put("/{conjunto_id}/torres/{torre_id}")
def update_torre(conjunto_id: int, torre_id: int, data: TorreUpdate, current_user: dict = Depends(get_current_user)):
    fields, values = [], []
    ...
    values.extend([torre_id, conjunto_id])
    with get_db() as conn:
        with conn.cursor() as cur:
            _check_conjunto_access(cur, current_user, conjunto_id)
            cur.execute(
                f"UPDATE torres SET {', '.join(fields)} WHERE id=%s AND conjunto_id=%s RETURNING *",
                values,
            )
            row = cur.fetchone()
            if not row:
...
@router.delete("/{conjunto_id}/torres/{torre_id}", status_code=204)
def delete_torre(conjunto_id: int, torre_id: int, current_user: dict = Depends(get_current_user)):
    with get_db() as conn:
        with conn.cursor() as cur:
            _check_conjunto_access(cur, current_user, conjunto_id)
            cur.execute(
            ...
            cur.execute("DELETE FROM torres WHERE id=%s AND conjunto_id=%s", (torre_id, conjunto_id))


# ── Unidades (vía torres) ─────────────────────────────────────────────────────

@router.get("/{conjunto_id}/unidades")
def get_unidades(conjunto_id: int, torre_id: Optional[int] = None):
    with get_db() as conn:
        with conn.cursor() as cur:
            query = """
                SELECT u.*,
...
@router.get("/{conjunto_id}/torres/{torre_id}/unidades")
def get_unidades_torre(conjunto_id: int, torre_id: int):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT u.*,
...
@router.post("/{conjunto_id}/unidades", status_code=201)
def create_unidad(conjunto_id: int, data: UnidadCreate, current_user: dict = Depends(get_current_user)):
    with get_db() as conn:
        with conn.cursor() as cur:
            _check_conjunto_access(cur, current_user, conjunto_id)
            cur.execute(
                "SELECT id FROM torres WHERE id = %s AND conjunto_id = %s",
                (data.torre_id, conjunto_id),
            )
            if not cur.fetchone():
                raise HTTPException(status_code=400, detail="La torre no pertenece a este conjunto")
...
@router.put("/{conjunto_id}/unidades/{unidad_id}")
def update_unidad(conjunto_id: int, unidad_id: int, data: UnidadUpdate, current_user: dict = Depends(get_current_user)):
    fields, values = [], []
    for field, val in data.model_dump(exclude_none=True).items():
        fields.append(f"{field} = %s")
        values.append(val)
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                """SELECT u.id FROM unidades u
                   JOIN torres t ON t.id = u.torre_id
                   WHERE u.id = %s AND t.conjunto_id = %s""",
                (unidad_id, conjunto_id),
            )
            if not cur.fetchone():
                raise HTTPException(status_code=404, detail="Unidad no encontrada en este conjunto")
...
@router.delete("/{conjunto_id}/unidades/{unidad_id}", status_code=204)
def delete_unidad(conjunto_id: int, unidad_id: int, current_user: dict = Depends(get_current_user)):
    with get_db() as conn:
        with conn.cursor() as cur:
            _check_conjunto_access(cur, current_user, conjunto_id)
            cur.execute(
            ...
            cur.execute(
                """DELETE FROM unidades WHERE id=%s AND torre_id IN (
                   SELECT id FROM torres WHERE conjunto_id=%s)""",
                (unidad_id, conjunto_id),
            )


@router.get("/{conjunto_id}/stats")
def get_stats(conjunto_id: int):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
